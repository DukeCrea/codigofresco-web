...
export const metadata = buildMetadata({
  title: t.casesIndex.metaTitle,
  description: t.casesIndex.metaDescription,
  path: "/en/cases",
  lang: "en",
  esPath: "/casos",
});
...
export default function EnglishCasesPage() {
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "CollectionPage",
        "@id": `${siteConfig.url}/en/cases#page`,
        name: t.casesIndex.collectionName,
        description: t.casesIndex.metaDescription,
        url: `${siteConfig.url}/en/cases`,
        inLanguage: "en",
      ...
      <main>
        <section className="border-b border-gray-900 bg-gray-950 px-6 py-16 md:px-8 md:py-20">
          <div className="mx-auto max-w-5xl">
            ...
            />
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-lime-300">Verifiable work</p>
            <h1 className="mt-4 text-4xl font-bold leading-tight md:text-6xl">{t.casesIndex.h1}</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-gray-300">{t.casesIndex.intro}</p>
          </div>
        </section>
        <section className="px-6 py-20 md:px-8">
          <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
            {enCaseStudies.map((item) => (
              <article key={item.slug} className="flex flex-col rounded-lg border border-gray-800 bg-gray-950 p-7">
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {item.stack.map((tech) => (
                    <span key={tech} className="rounded-full border border-gray-700 px-3 py-1 text-xs text-gray-200">{tech}</span>
                  ))}
                </div>
                <Link href={`/en/cases/${item.slug}`} className="mt-7 inline-flex items-center gap-2 font-bold text-lime-300 underline underline-offset-4 hover:text-lime-200">
                  Read the full case <CheckIcon className="h-4 w-4" />
                </Link>
              </article>
            ))}
          </div>
